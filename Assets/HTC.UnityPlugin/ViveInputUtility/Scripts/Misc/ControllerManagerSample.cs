...
public class ControllerManagerSample : MonoBehaviour
{
    ...
    protected virtual void LateUpdate()
    {
        ...
        switch (curvePointerActiveMode)
        {
            ...
            case CurvePointerActiveModeEnum.ActiveOnPadPressed:
                needUpdate |= SetRightCurvePointerActive(ViveInput.GetPress(HandRole.RightHand, ControllerButton.Pad));
                needUpdate |= SetLeftCurvePointerActive(ViveInput.GetPress(HandRole.LeftHand, ControllerButton.Pad));
                break;

            case CurvePointerActiveModeEnum.ToggleByPadDoubleClick:
                if (ViveInput.GetPressDown(HandRole.RightHand, ControllerButton.Pad) && ViveInput.ClickCount(HandRole.RightHand, ControllerButton.Pad) == 2)
                {
                    ToggleRightCurvePointer();
                    needUpdate = true;
                }

                if (ViveInput.GetPressDown(HandRole.LeftHand, ControllerButton.Pad) && ViveInput.ClickCount(HandRole.LeftHand, ControllerButton.Pad) == 2)
                {
        ...
        switch (customModelActiveMode)
        {
            ...
            case CustomModelActiveModeEnum.ActiveOnGripped:
                needUpdate |= SetRightCustomModelActive(ViveInput.GetPress(HandRole.RightHand, ControllerButton.Grip));
                needUpdate |= SetLeftCustomModelActive(ViveInput.GetPress(HandRole.LeftHand, ControllerButton.Grip));
                break;

            case CustomModelActiveModeEnum.ToggleByDoubleGrip:
                if (ViveInput.GetPressDown(HandRole.RightHand, ControllerButton.Grip) && ViveInput.ClickCount(HandRole.RightHand, ControllerButton.Grip) == 2)
                {
                    ToggleRightCustomModel();
                    needUpdate = true;
                }
                if (ViveInput.GetPressDown(HandRole.LeftHand, ControllerButton.Grip) && ViveInput.ClickCount(HandRole.LeftHand, ControllerButton.Grip) == 2)
                {
    ...
    {
        var viveEventData = grabbedObj.grabbedEvent as ViveColliderButtonEventData;
        if (viveEventData == null) { return; }
